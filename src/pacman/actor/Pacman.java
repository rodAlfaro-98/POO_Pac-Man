...
public class Pacman extends PacmanActor {
    ...
    public Pacman(PacmanJuego game) {
        super(game);
    }
    ...
    public void init() {
        ...
        for (int d=0; d<4; d++) {
            for (int i=0; i<4; i++) {
                pacmanFrameNames[i + 4 * d] = "/Sprites/pacman_" + d + "_" + i + ".png";
            }
        }
        for (int i=0; i<14; i++) {
            pacmanFrameNames[16 + i] = "/Sprites/pacman_died_" + i + ".png";
        }
    ...
    public void updatePlaying() {
        ...
        if (Teclado.TeclaPresionada[KeyEvent.VK_LEFT]) {
            desiredDirection = 2;
        }
        else if (Teclado.TeclaPresionada[KeyEvent.VK_RIGHT]) {
            desiredDirection = 0;
        }
        else if (Teclado.TeclaPresionada[KeyEvent.VK_UP]) {
            desiredDirection = 3;
        }
        else if (Teclado.TeclaPresionada[KeyEvent.VK_DOWN]) {
            desiredDirection = 1;
        }
        ...
        while (true) {
            switch (instructionPointer) {
                case 0:
                    ...
                    if (game.Laberinto[row + dy][col + dx] == 0) {
                        direction = desiredDirection;
                    } 
                    // anexamos metodo de funciones trigonometricas para realizar algunas conversiones
                    angle = Math.toRadians(direction * 90);
                    dx = (int) Math.cos(angle);
                    dy = (int) Math.sin(angle);
                    if (game.Laberinto[row + dy][col + dx] == -1) {
                        break yield;
                    } 
