...
import pacman.infra.PatronDeCaminoCorto;
...
public class Ghost extends PacmanActor {
    ...
    // Modo para devolver al fantasma al centro del mapa
    public PatronDeCaminoCorto pathFinder;

    //ConatructorFantasma
    public Ghost(PacmanJuego game, Pacman pacman, int type) {
        super(game);
        this.pacman = pacman;
        this.type = type;
        this.pathFinder = new PatronDeCaminoCorto(game.Laberinto);
    }
    ...
    public void init() {
        ...
        for (int i = 0; i < 8; i++) {
            ghostFrameNames[i] = "/Sprites/ghost_" + type + "_" + i + ".png";
        }
        for (int i = 0; i < 4; i++) {
            ghostFrameNames[8 + i] = "/Sprites/ghost_vulnerable_" + i + ".png";
        }
        for (int i = 0; i < 4; i++) {
            ghostFrameNames[12 + i] = "/Sprites/ghost_died_" + i + ".png";
        }
    ...
    private void updateGhostNormal() {
        ...
        if (type == 0 || type == 1) {
            updateGhostMovement(true, pacman.col, pacman.row, 1, pacmanCatchedAction, 0, 1, 2, 3); // movimiento de persecucion
        } else {
            updateGhostMovement(false, 0, 0, 1, pacmanCatchedAction, 0, 1, 2, 3); //movimiento random
        }
    }
    ...
    private void updateGhostVulnerable() {
        ...
        updateGhostMovement(true, pacman.col, pacman.row, 1, ghostCatchedAction, 2, 3, 0, 1); // movimiento de huir
        // Volver al modo normal despues de 8 segundos
    ...
    private void updateGhostDied() {
        ...
        while (true) {
            switch (instructionPointer) {
                case 0:
                    pathFinder.Buscar(col, row, 18, 11);
                    instructionPointer = 1;
                case 1:
        ...
        while (true) {
            switch (instructionPointer) {
                case 0:
                    ...
                    if (useTarget && game.Laberinto[row + dy][col + dx] == 0
                        ...
                        do {
                            ...
                            dx = (int) Math.cos(angle);
                            dy = (int) Math.sin(angle);
                        } while (game.Laberinto[row + dy][col + dx] == -1
    ...
    public void stateChanged() {
        if (game.getState() == PacmanJuego.State.TITULO) {
            ...
            visible = false;
        } else if (game.getState() == PacmanJuego.State.READY2) {
            setMode(Mode.CAGE);
            updateAnimation();
            Point initialPosition = initialPositions[type];
            updatePosition(initialPosition.x, initialPosition.y); // col, row
            x -= 4;
        } else if (game.getState() == PacmanJuego.State.PLAYING && mode != Mode.CAGE) {
            instructionPointer = 0;
        } else if (game.getState() == PacmanJuego.State.PACMAN_DIED) {
            instructionPointer = 0;
        } else if (game.getState() == PacmanJuego.State.LEVEL_CLEARED) {
            instructionPointer = 0;
        }
    }
